...
def parse_event(e_filedir, e_filename):
...
if __name__ == '__main__':
    ...
    events = parse_event(e_filedir, e_filename)
    ...
    f_start_index = 98   # the timestamp index is same to the name of frame
    print('f_start_index:',f_start_index,'total timestamps:',len(frame_timestamps))  # 500000-27 1000000-54
    if frame_timestamps <= f_start_index:
    ...
    # print(f_start_index-2)  # collect events for the first reconimage of two frame timestamp
    ...
    for f_ts in frame_timestamps[f_start_index:f_start_index + 50]:
        # load frame img
        ...
        img_out = img_normalization(img_out)
        print('show end')

        save_path = '/Users/cainan/Desktop/Project/data/processed/cutoff_120' + '/'
        cv2.imwrite(save_path + 'event' + str(f_index) + '.png',img_out)
        if f_index >= 109:
            k = cv2.waitKey(0)
            if k == 27:         # ESC
